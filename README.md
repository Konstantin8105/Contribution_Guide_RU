...
* [Внесение вклада (Making a Contribution)](#Внесение-вклада-making-a-contribution)
	...
	* [Внесение изменений](#Внесение-изменений)
		* [Загрузка исходных кодов проекта Go](#Загрузка-исходных-кодов-проекта-go)
		* [Добавление в основное дерево Go](#Добавление-в-основное-дерево-go)
...
Посестите [go.googlesource.com](https://go.googlesource.com) и кликните на  "Generate Password" в верхней правой части меню страницы.
Вас потом перенаправит на accounts.google.com для авторизации.

### Шаг 2: Запуск скрипта

После регистрации, Вы попадете на страницу [go.googlesource.com](https://go.googlesource.com) озаглавленную "Configure Git"(Настройка Git).
Эта страница включает область с Вашим персональным скриптом, который при локальном запуске (на Вашем компьютере) настроет в git для Вас уникальный аутентификационный ключ.
Это ключ является одним из пары, причем один генерируется на стороне сервера, совсем как с ssh.

Скопируйте и запустите этот скрипт в Вашем терминале.
...
### Какое CLA (Сontributor License Agreement)
...
команда должна вывести текст помощи, не ошибку.
...
Даже если Вы собираетесь использовать простой Git для ежедневной работы, то настройте Git checkout запустив `git-codereview` `hooks`.
...
Подкоманда `git-codereview` была выбрана таким образом, чтобы отличилась от собственных команд Git, так что это более безопастно.
...
Проект приветствует Вашу работу, но пожайлуста дайте знать о том над чем Вы работаете в случае,  если хотите что-то изменить или добавить в репозитории Go.
...
При планировании Вашей работы, пожайлуста следите за [шестимесячным циклом разработки проекта Go](https://golang.org/wiki/Go-Release-Cycle).
...
### Загрузка исходных кодов проекта Go
...
Зайдите в папку, куда хотели бы выгрузить исходные коды и запустите следующую команду в теминале.
...
Неибольшее количество установок Go использует ветку release, но новые изменения должны основываться на ветве master.
...
Затем Вы можете зайти в папку пакета (`$GOPATH/src/golang.org/x/oauth2`).
...
Сделайте Ваши изменения как Вы сочтете нужным, добавьте тестов для Ваших изменений. Проверьте Ваши изменения как Вам кажется нужно.
...
Данная строка будет использоваться для однострочного суммарного изменения, префиксом для основного изменяемого пакета, а также будет использоваться в качестве темы электронных писем при рассмотрении кода.
...
Запись должна производиться с правильной пунктуацией, как и Ваши комментарии в Go.
Если у Вас есть поясняющая ссылка, то добавьте её.
Если Вы исправляете проблему (issue), то укажите её номер прежде всего.

После редактирования, можно будет читать:
...
Закомментированая секция файла со списком всех измененных файлов.
...
Для примера, если в сообщении фиксации добавить Fixes #45, то проблема 45 в этом репозитории будет слита в ветку по-умолчанию.
Ключевые слова:
close
closes
closed
...
Закончив писать сообщение фиксации (the commit message), сохраните файл и выйдите из редактора.

Вы должны быть уверены, что имеете в своей среде переменную $EDITOR и она работает правильно для успешного завершения.
...
для обновления описания изменения и включения поэтапных измененияй.
...
Вы должны [написать и протестировать свой код](https://golang.org/doc/code.html), но перед отправкой кода на рассмотрение, запустите Ваши тесты для всего дерева для того, чтобы убедиться, что Ваше изменение не сломает другие пакеты и программы:
...
Это делается с помощью настройки псевдонима `mail`, который, не смотря на своё имя, не отправляет почту, а лишь отправляет изменения в Gerrit, как git push.
...
Если Ваше изменение связано с открытой проблемой (issue), то добавьте комментарий в проблему(issue) о предлагаемом исправлении, включая ссылку на Ваш CLA.
...
Все изменения автоматически включаются в список рассылки [golang-codereviews@googlegroups.com](https://groups.google.com/group/golang-codereviews). Если это ваше первое изменение, возможна задержка перед появлением в списке рассылки, это сделано для предотвращения спама в рассылке.

Вы можете указать рецензента или заинтересованные стороны CC, используя опции `-r` или` -cc`.
...
(К сожалению, использование старой системы рассмотрений Rietveld, отвечающая на письма, показывала свою не эффективность.)
...
Редко, что первоначальное изменение будет готово к применению, как есть.
...
Рецензент одобряет изменение, давая ему положительный результат (+1 или +2) и отвечающий `LGTM`: выглядит хорошо.
...
Эти изменения проверяються в репозитории.
